...
左のエディタに Markdown を書き、`---` でスライドを区切って、右上の書き出しボタンで
`.pptx` または `.pdf` がダウンロードされます（**PPTX / PDF** はトグルで選択）。PDF は各スライドを
画像化して 1 ページずつ埋め込みます（Markdown タブは Marp の実描画、ビジュアルタブは編集中の deck）。
