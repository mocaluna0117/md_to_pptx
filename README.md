...
## スタイル調整（プレビュー上部のコントロール）

テーマ / ページ番号 / 背景色 / 文字色 を UI から変更できます。変更は **Marp のフロントマター
ディレクティブとして Markdown に書き戻される**ため、エディタにも反映され、Markdown が唯一の
元データのまま保たれます。プレビューと方式A（画像）に即反映され、背景色・文字色は方式B（編集
可能）の書き出しにも反映されます。
...
- [src/lib/frontmatter.ts](src/lib/frontmatter.ts) — フロントマター（Marp ディレクティブ）の読み書き
- [src/App.tsx](src/App.tsx) — エディタ / ライブプレビュー / スタイル調整 / 方式トグル / 書き出し UI
