...
メニューが開き、書き出し形式を選んでダウンロードできます。メニューは表示タブで変わります。

- **Markdown タブ**：**PPTX（見た目そのまま）** … Marp の見た目を画像として忠実に再現 ／
  **PPTX（編集可能テキスト）** … PowerPoint で文字を編集（簡易レイアウト）／ **PDF**
- **ビジュアル編集タブ**：**PPTX（編集可能）** … 各ボックスをネイティブのテキストボックスに ／ **PDF**

PDF は各スライドを画像化して 1 ページずつ埋め込みます（Markdown タブは Marp の実描画、
ビジュアルタブは編集中の deck）。「見た目そのまま」はプレビューをそのままラスタライズするため、
表示中のプレビューと一致します。

既存の `.md` を使う場合は、エディタ上部の **「📂 ファイルをインポート」**（対応: `.md` / `.markdown` / `.txt`）から選ぶか、エディタに **ドラッグ&ドロップ**でインポートできます（ファイル名は出力名にも反映）。

画面右上のツールバーの **「🔄 初期化」**（「ビジュアルに反映」の隣）で、すべてをデフォルトのサンプル
（インポート前の状態）に戻せます（確認あり・自動保存もクリア）。

## 表示モード（ツールバー左の切替）

- **Markdown**：エディタ＋ライブプレビュー（従来の画面）。書き出しは見た目そのまま（画像）／編集可能テキストの PPTX、または PDF。
...
- 文字サイズ・揃え、ボックスの追加／削除、スライドの追加／削除
- **Undo / Redo**（ツールバーの ↶ / ↷、または Ctrl/⌘+Z・Ctrl/⌘+Shift+Z）。ドラッグ 1 回＝Undo 1 回
...
## PPTX の書き出し方式（「書き出す ▾」メニューで選択）

かつてのツールバー上の方式トグルは廃止し、書き出し時に選ぶ方式にしました。
編集可能な PPTX が欲しいときは、レイアウト再現に優れる**ビジュアル編集タブからの書き出し**を推奨します。

| 方式 | どこから | 見た目 | PowerPoint での編集 | 用途 |
| --- | --- | --- | --- | --- |
| **見た目そのまま（画像）** | Markdown タブ | Marp の見た目そのまま（高再現・プレビューと一致） | 不可（スライド＝画像） | 見た目重視・そのまま配布 |
| **編集可能テキスト** | Markdown タブ | 簡易（自前レイアウト） | 可能（本物のテキスト/表） | 手早く編集用の下書き |
| **編集可能（deck）** | ビジュアル編集タブ | 編集した見た目に近い | 可能（各ボックス＝テキストボックス） | あとから PowerPoint で仕上げる |
...
- [src/App.tsx](src/App.tsx) — 表示タブ切替 / エディタ / プレビュー / 書き出しメニュー UI
