...
画像が **相対パス参照**（例: `![](fig1.png)`）の Markdown は、そのままではブラウザから画像ファイルを
参照できません。ドロワーの **「🖼 画像」** ボタン（またはドロワーへ画像をドラッグ&ドロップ）でその画像
ファイルを読み込むと、**ファイル名一致**で参照が解決され、表示・書き出しに反映されます（`data:` の
データ URI 埋め込み画像はそのまま使えます）。Docdown も同じ方式に対応しています。
...
- **数式（LaTeX）** `$…$` / `$$…$$` は KaTeX で描画して画像化し、スライド・Word・PDF に反映（Deckdown / Docdown 共通）
